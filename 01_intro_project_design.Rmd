...
### Variable names    
As part of controlled vocabulary, the way you type the variable names is also important and also important to be consistent. I have found **snake case** to be the best for me and using the package **Janitor** works well
to fix messy file names.  The main problem is variable names with spaces requires back
ticks (`variable name`) to use them and special characters (`!@#$%^&* and u(micro) etc.) 
are coded differently on different operating systems and create  chaos. The different styles
are below: 
